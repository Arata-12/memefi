...
# Handle all errors here, bro. SAFE_POST.
    ...
    for attempt in range(retries):
        try:
            ...
            if res.status == 200:
                ...
                print(f"❌ {termcolor.colored(f" Failed with status {res.status}, trying again","red")}")
...
def cek_user(index):
    ...
    headers = headers_set.copy()  # Create a copy of headers_set to avoid modifying the global variable
...
def activate_booster(index, headers):
    ...
    headers = headers_set.copy()  # Creating a copy of headers_set to avoid modifying the global variable
    headers['Authorization'] = f'Bearer {access_token}'
                ...
                if tap_result is not None:
                    if 'data' in tap_result and 'telegramGameProcessTapsBatch' in tap_result['data']:
                        ...
                        if tap_data['currentBoss']['currentHealth'] == 0:
                            print(f"\n {termcolor.colored("The boss has been defeated, setting up the next boss...", "green")}")
                            set_next_boss(index, headers)
...
def submit_taps(index, json_payload):
    ...
    response = safe_post(url, headers, json_payload)
    if response:
        return response  # Ensure returning the parsed data
    else:
        print(f"❌ Failed with status {response}, trying again...")
        return None  # Return None if an error occurs
...
def check_and_complete_tasks(index, headers):
    ...
    if all_completed:
        print(f"\r[ Account {index + 1} ] All tasks are completed. ✅            ", flush=True)
        return True
    ...
    for task in tasks['data']['campaignTasks']:
        ...
        if task['name'] == "Follow telegram channel" and task['status'] == "Pending":
            ...
            continue  # Skip the task if the task name is "Follow telegram channel" and its status is "Pending"
    ...
    # Check the tasks again after moving to verification
    ...
    while True:
        ...
        for index, result, first_name, last_name, league in accounts:
            ...
            if stat_result is not None:
                ...
                if darah_bos == 0:
                    print("\n Boss has been defeated, setting the next boss...", flush=True)
                    set_next_boss(index, headers)
...
# Run the main() function and save the result
...
def claim_combo(index, headers):
    ...
    if response and 'data' in response and 'telegramGameProcessTapsBatch' in response['data']:
        ...
        print("❌ Failed to claim combo: Incomplete data or none available.")
...
if turbo_booster == 'y':
    while True:
        instant_kill_Mode = input("Activate instant kill Mode (1x tap monster dead) ? (y/n): ").strip().lower()
        if instant_kill_Mode in ['y', 'n', '']:
            instant_kill_Mode = instant_kill_Mode or 'n'
            break
        else:
            print("Enter 'y' or 'n'.")
